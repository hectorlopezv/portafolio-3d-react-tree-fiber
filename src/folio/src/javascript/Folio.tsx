...
const Folio = () => {
  ...
  const setPasses = useCallback(() => {
    ...
    passes.current.glowsPass.material.uniforms.uColor.value = new THREE.Color(passes.current.glowsPass.color).convertLinearToSRGB();
    ...
    if (debug.current) {
      ...
      folder.addColor(passes.current.glowsPass, 'color').name('color').onChange(() => {
        passes.current.glowsPass.material.uniforms.uColor.value = new THREE.Color(passes.current.glowsPass.color).convertLinearToSRGB();
      });
    ...
    sizes.current.on('resize', () => {
      renderer.setSize(sizes.current.viewport.width, sizes.current.viewport.height);
      passes.current.composer.setSize(sizes.current.viewport.width, sizes.current.viewport.height);
  ...
  const setTitle = useCallback(() => {
    ...
    time.current.on('tick', () => {
      if (world.current.physics) {
        title.current.absolutePosition += world.current.physics.car.forwardSpeed;
    ...
    window.setInterval(() => {
      title.current.position = Math.round(title.current.absolutePosition % title.current.width);
      document.title = `${'_'.repeat(title.current.width - title.current.position)}🚗${'_'.repeat(title.current.position)}`;
